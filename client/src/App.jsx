...
export default function App() {
  ...
  useEffect(() => {
    ...
    s.on('room_state', r => {
      ...
      else if (r.gameState === 'lobby') setScreen('lobby');
  ...
  useEffect(() => {
    ...
    // Auto scroll if user was already at bottom or if it's their own message
    const lastMsg = messages[messages.length - 1];
    if (isAtBottom || lastMsg?.username === username) {
      scrollToBottom();
    } else {
  ...
  const isHost = players.find(p => p.status === 'online')?.id === socket?.id;
  ...
  return (
    <div className="h-screen w-screen overflow-hidden bg-[#010101] text-white font-inter relative">
      ...
      <AnimatePresence mode="wait">
        {!isConnected && screen !== 'landing' && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
            className="fixed inset-0 z-[100] bg-black/90 backdrop-blur-xl flex flex-col items-center justify-center gap-4 p-6 text-center">
            <div className="relative">
              <Loader2 className="w-10 h-10 text-electric-purple animate-spin" />
              <div className="absolute inset-0 blur-xl bg-electric-purple/20 animate-pulse" />
            </div>
            <div>
              <p className="text-[10px] font-black uppercase text-white/50 tracking-[0.3em] mb-1">Connecting to Server</p>
              <p className="text-[8px] font-bold text-white/20 uppercase">Render cold-start may take 30s ⚡</p>
            </div>
          </motion.div>
        )}
          ...
          {screen === 'game' && (
            <div className="flex flex-col h-full overflow-hidden relative">
                    ...
                    <motion.div key="sel" {...fade} className="absolute inset-0 flex flex-col items-center justify-center gap-4 px-5">
                      ...
                      {!isMyTurn && (
                        <div className="flex flex-col items-center gap-3">
                          <p className="text-[8px] uppercase font-black text-white/15 animate-pulse">Waiting for {players[currentTurn]?.name}…</p>
                          {isHost && players[currentTurn]?.status !== 'online' && (
                            <button onClick={skipTurnAct} className="px-5 h-10 bg-red-500/10 border border-red-500/30 rounded-xl text-[10px] font-black uppercase text-red-400 hover:bg-red-500/20 transition-all">
                              Skip Offline Player 👋
                            </button>
                          )}
                        </div>
                      ...
                      <div className="flex-1 flex flex-col gap-2 min-w-0">
                        ...
                        <p className="text-sm font-black italic uppercase tracking-tight leading-snug line-clamp-2">"{question}"</p>
                          <div className="flex gap-2">
                            <div className={`flex gap-2 flex-1 ${!isMyTurn ? 'opacity-25 pointer-events-none' : ''}`}>
                            ...
                            {!isMyTurn && isHost && players[currentTurn]?.status !== 'online' && (
                              <button onClick={skipTurnAct} className="h-8 px-3 bg-red-500/10 border border-red-500/30 rounded-xl text-[10px] font-black uppercase text-red-400 hover:bg-red-500/20 transition-all">
                                Skip ⏭️
                              </button>
                            )}
                          </div>
              ...
              <div className="flex flex-col flex-1 min-h-0 bg-[#050505]">
                ...
                {/* Messages */}
                <div className="flex-1 overflow-y-auto px-4 py-3 space-y-2 no-scrollbar relative" ref={chatRef} onScroll={handleChatScroll}>
                  ...
                  <AnimatePresence>
                    {showScrollBottom && (
                      <motion.button
                        ...
                        className="fixed bottom-20 left-1/2 -translate-x-1/2 z-20 px-4 py-2 bg-electric-purple rounded-full shadow-2xl flex items-center gap-2 border border-white/20 active:scale-95 transition-all"
                      >
                        <ArrowLeft className="w-3 h-3 rotate-[-90deg]" />
                        <span className="text-[10px] font-black uppercase tracking-widest text-white">Latest Messages</span>
                      </motion.button>
                    )}
                  </AnimatePresence>
                  {messages.length === 0 && (
                  ...
                  {messages.map((m, i) => {
                    ...
                    return (
                      <div key={m.id || i} id={`msg-${m.id}`} className={`flex flex-col ${isMine ? 'items-end' : 'items-start'} group max-w-full`}>
                        {/* Sender info */}
                          ...
                          <motion.div
                            ...
                            {quoted && (
                              <div 
                                ...
                                className="mb-2 p-2 bg-black/20 border-l-2 border-white/40 rounded-md text-[10px] opacity-70 line-clamp-1 italic select-none cursor-pointer hover:bg-black/40 transition-colors">
